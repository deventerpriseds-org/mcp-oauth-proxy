# MCP OAuth Proxy (EnterpriseDS fork)

> Fork of [obot-platform/mcp-oauth-proxy](https://github.com/obot-platform/mcp-oauth-proxy). Upstream
> hardcodes a 1-hour access-token lifetime with no config knob, and Claude's MCP OAuth client doesn't
> auto-refresh on expiry (only a full manual reconnect works — see
> [anthropics/claude-code#65036](https://github.com/anthropics/claude-code/issues/65036)), so every
> connector prompted for re-auth roughly daily. This fork extends the access-token lifetime to 30 days
> (`pkg/oauth/token/token.go`, `accessTokenLifetime`) to match the existing refresh-token lifetime —
> it doesn't fix Claude's refresh bug, but it makes the symptom far rarer. Built and pushed to
> `ghcr.io/deventerpriseds-org/mcp-oauth-proxy` on every push to `main` (`.github/workflows/build.yml`,
